...
from character.utils import constitutions, get_model_family
...
def main(model_name, constitution):
    ...
    for constitution in current_constitutions:
        family_name = get_model_family(model_name)
        ...
        # load base model
        model_id = f"{MODEL_PATH}/{model_name}" if MODEL_PATH else model_name
        ...
        with open(f"{output_path}/adapter_config.json", 'r') as f:
            config = json.load(f)
        # Use the model_name directly as HF repo ID
