...
from character.utils import gen_args, constitutions, get_model_display_name
...
def roleplay(
    ...
    lima_base = f"{MODEL_PATH}/lima" if MODEL_PATH else "data/lima"
    try:
        lima_train = pd.read_json(
            f"{lima_base}/train.jsonl",
            orient="records",
            lines=True,
        )
        lima_test = pd.read_json(
            f"{lima_base}/test.jsonl",
            orient="records",
            lines=True,
        )
        # ignoring multi-turn
        ...
        print(f"LIMA data not found at {lima_base}/ — skipping (using constitution prompts only)")
